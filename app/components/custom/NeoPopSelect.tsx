  ...
  return (
    <div className={cn("relative w-full", className)} ref={dropdownRef}>
      ...
      >
        ...
        <span className={cn("text-[10px] transition-transform duration-200 text-[#666]", isOpen && "rotate-180 text-[var(--neopop-white)]")}>
      ...
      {isOpen && (
        <div className="absolute top-[calc(100%_+_8px)] left-0 right-0 z-[100] max-h-[200px] overflow-y-auto bg-[var(--cred-black)] border border-[var(--cred-border)] shadow-[8px_8px_0px_rgba(0,0,0,0.8)] animate-slideDown">
          {options.map((option) => (
            <div 
              key={option}
              className="w-full text-left bg-transparent border-none border-b border-[#1a1a1a] text-[#cccccc] p-4 px-5 cursor-pointer transition-all duration-100 ease-linear text-sm block hover:bg-[#1C1C1C] hover:text-white hover:pl-6"
              onClick={() => {
                onChange(option);
                setIsOpen(false);
              }}
